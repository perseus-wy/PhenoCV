# -*- coding: utf-8 -*-
"""Pluggable trait-extractor registry for PhenoCV phenotyping.
...
* declares its **input dependencies** (``mask`` / ``rgb`` / ``depth`` /
  ``calibration``) and a **tier** (1=mask-only, 2=mask+rgb, 3=depth+calib,
  4=multispectral);
...
tier order, and merges the result into one row. Adding a new phenotype tool is
therefore **just**: write one extractor, decorate it with ``@register`` — no
engine changes. This is the extension point that lets PhenoCV grow into a
general plant-phenotyping CV toolkit.
"""
